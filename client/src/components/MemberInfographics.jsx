...
const FLOW_COLORS = ['#6366f1', '#ef4444', '#0ea5e9'];
...
export default function MemberInfographics({ totals, formatCurrency }) {
  ...
  const flowData = buildFlowData(totals);
  ...
  const hasFlowData = flowData.some((entry) => entry.value > 0);
  ...
  return (
    <section className="rounded-[2rem] border border-slate-200 bg-white p-5 shadow-sm sm:p-6">
      ...
      <div className="grid grid-cols-1 gap-4 2xl:grid-cols-3 xl:grid-cols-2">
        ...
        <ChartPanel
          title="Money Flow"
          helper="Compare deposits, withdrawals, and loan repayments to understand the member’s overall movement of funds."
        >
          <div className="mb-4 flex items-start justify-between gap-3">
            <div>
              <p className="text-xs font-semibold uppercase tracking-[0.18em] text-slate-400">Cash Movement</p>
          ...
          {hasFlowData ? (
            <div className="h-56">
              <ResponsiveContainer width="100%" height="100%">
                <BarChart data={flowData} margin={{ top: 8, right: 8, left: -10, bottom: 0 }}>
                  <CartesianGrid strokeDasharray="3 3" stroke="rgba(148,163,184,0.18)" />
                  <XAxis dataKey="name" tick={{ fontSize: 12, fill: '#64748b' }} axisLine={false} tickLine={false} />
                  <YAxis
                  ...
                  <Bar dataKey="value" radius={[10, 10, 0, 0]}>
                    {flowData.map((entry, index) => (
                      <Cell key={entry.name} fill={FLOW_COLORS[index % FLOW_COLORS.length]} />
                    ))}
                  </Bar>
                </BarChart>
              </ResponsiveContainer>
            ...
            <p className="rounded-2xl bg-white px-4 py-10 text-center text-sm text-slate-500">Cash-flow insights will appear here after savings or repayment records are added.</p>
          )}
            ...
            {flowData.map((entry, index) => (
              <div key={entry.name} className="rounded-2xl bg-white px-3 py-3">
                <div className="flex items-center gap-2">
                  <span
                    className="inline-flex h-2.5 w-2.5 rounded-full"
                    style={{ backgroundColor: FLOW_COLORS[index % FLOW_COLORS.length] }}
                  />
                  <p className="text-xs font-semibold uppercase tracking-[0.16em] text-slate-400">{entry.name}</p>
                </div>
                <p className="mt-2 text-sm font-semibold text-slate-900">{formatCurrency(entry.value)}</p>
